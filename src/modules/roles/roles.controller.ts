import { PresenceMode } from '@hapi/joi'
import {
  BadRequestException,
  Body,
  Controller,
...
export class RolesController {
  ...
  @Get('/paginated')
  @HttpCode(HttpStatus.OK)
  async paginate(@Query('page') page: number): Promise<PaginatedResult> {
  ...
  async create(@Body() createRoleDto: CreateUpdateRoleDto, @Body('permissions') permissionsIds): Promise<Role> {
    /* map()
            [1,2]
            [{id: 1}, {id: 2}]
        */
  ...
  async update(
    ...
    @Body('permissions') permissionsIds,
  ): Promise<Role> {
